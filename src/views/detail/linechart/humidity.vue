<template>
  <div class="container">
    ...
    <div id="humLine" style="width: 760px;height:250px;" />
  </div>
</template>

<script>
import echarts from 'echarts'
import { getHumidity } from '@/api/service'
...
export default {
  ...
  watch: {
    humLineOption: {
      handler(newVal, oldVal) {
        if (this.humLine) {
          if (newVal) {
            this.humLine.setOption(newVal)
          } else {
            this.humLine.setOption(oldVal)
          }
        } else {
          this.getTable()
        }
  ...
  methods: {
    getTable() {
      this.humLine = echarts.init(document.getElementById('humLine'))
      this.humLine.setOption(this.humLineOption)
    },
    async getData() {
      var query = {}
      var date = new Date()
      ...
      const { data: res } = await getHumidity(query)

      res.data.forEach(item => {
        humData.push(item * 1)
      })

      this.humLineOption.series[0].data = humData
      this.humLineOption.xAxis.data = time.reverse()

      // console.log(humData)
      // console.log(this.humLineOption.series[0].data)
    },
    chooseSeven() {
      this.days = '7'
      this.isshow = true
